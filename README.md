...
- **String definitions** (jora queries):
  ...
  String definitions have access to special references such as `$`, `$$`, `@`, and `#`. Additionally, all built-in methods and assertions, along with any custom elements, are accessible within these definitions.

  > **Note**: The `@` reference pertains to the entry value of a method or an assertion definition, and not to the entry value of the query itself.
